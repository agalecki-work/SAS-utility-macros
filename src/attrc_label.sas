%macro attrc_label(member, libname = work) / des = "M1: Returns label of the dataset ";
...
%do;
%let attrc = %sysfunc(attrc(&DSID, LABEL)); /* Dataset label */
%if %length(&attrc) = 0 %then %let attrc = Label not available; 
%end;
%mexit:
